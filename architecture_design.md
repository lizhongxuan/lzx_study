...
> 面试官，我对**首页高频配置读取**场景下，做了**多级缓存架构**的优化。主要是针对**单一热点 Key 把 Redis 网卡打满**的问题，借鉴 CPU L1/L2 缓存思想，引入了**进程内缓存 (Local Cache)**。最终达到的效果是**将 90% 的流量拦截在应用内部，将热点读取延迟从 50ms 降低到了 0.1ms**。
